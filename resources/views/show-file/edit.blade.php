                ...
                <div class="row" style='width: 500px; margin: auto; padding: 5px; margin-top: 5px;'>
                    <div class="col-md-6" style='padding: 0px;'>
                        <div class="form-group">
                            ...
                            <input type="text" class="nameNew" placeholder='Name' name="nameNew" style='outline: none; border-radius: 5px; border: 2px solid #46b4f7; padding: 2px 4px;' class="form-control" value="{{ $file->name }}" placeholder="Nhập họ tên">
                        </div>
                    </div>
                    <div class="form-group" style='padding: 0px; margin-top: 5px; margin-bottom: 0'>
                        ...
                        <div style="display: flex; margin-bottom: 10px">
                            <input type="text" name="nameFile[]" id="nameFile" oninput="handleReadonly(this)" style='border: 2px solid #46b4f7;' class="form-control check1" value="{{ $item->getFilename() }}" data-realpart="{{ $item->getRealPath() }}" placeholder="Nhập họ tên">
                            <div style="display: flex;"> <a type="button" style='margin: 0 2px; ' href="http://127.0.0.1:8000/storagefile/download?path={{ $item->getFilename() }}&&name={{ $file->name }}&&id={{ $file->id }}" class="btn btn-info">Xem</a>
                                <a type="button" style='margin: 0 2px;' href="http://127.0.0.1:8000/storagefile/delete?path={{ $item->getFilename() }}&&name={{ $file->name }}&&id={{ $file->id }}" class="btn btn-danger">Xóa</a>
                            </div>
                        </div>
                        @endforeach
                    </div>
                    <div class="d-flex" style='padding: 0px; margin-top: 5px;    margin-bottom: 0  '>
                        <div class="custom-files" style="flex: 1;position: relative;">
                            <div class="custom-file custom-file0" style='position: relative;' data-index='0'>
                                <input type="file" hidden name="files[]" onchange="displayFileName(this)" data-index='0' class="onInput form-control " id="chooseFileQ" multiple>
                                <input type="text" name="name" class="form-control replaceName" data-index='0' oninput='handleLoadText2(this)' placeholder="Tên dự án" style='z-index: -3; position: absolute; width: 73%; padding: 5px; border: 2px solid #46b4f7; border-radius: 5px;'>
                                <label class="custom-file-label labelName" for="chooseFileQ" data-index='0' style='width: 88%; padding: 5px; border: 2px solid #46b4f7; border-radius: 5px; z-index: 3;'>Select
                                    file</label>
                            </div>
                            <button style="height: fit-content; position:absolute;  width: fit-content ; right: 2px; top: 0px; font-size: 25px; padding: 0px 18px; display: flex; align-items: center;" class="addtextbox btn-info border-0 rounded-1 rounded text-capitalize" type="button">+</button>
                    ...
                    <div style=" width: 100%; text-align: center;border-radius: 5px;border: 2px solid #46b4f7;  padding: 2px 4px;  margin-top: 10px;">
                        <div class="text-center" style="margin-top: 50px;">
                            <div>
                                <div class="card-body" style='    width: 100px; height: 100px; margin: auto; padding: 0;'>
